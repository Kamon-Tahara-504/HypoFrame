...
| フェーズ | 内容 | 確認の目安 |
|----------|------|------------|
| 0 | 環境・型・骨組み | 型とディレクトリが揃い、開発サーバーが起動する |
| 1 | クロール（HP取得） | URL からテキストが取得できる |
| 2 | LLM・プロンプト（要約・仮説・提案文） | 要約・仮説5段・提案文が Groq で生成できる |
| 3 | API `/api/generate` | 1 リクエストでクロール→要約→仮説→提案文が返る。タイムアウト・エラー形式も実装 |
| 4 | DB（Supabase） | マイグレーション・保存・編集 API が動く |
| 5 | UI 骨組み | 1 画面で入力→ローディング→結果表示→エラー表示まで見える |
| 6 | 編集・再生成・エクスポート・コピー | 編集・再生成1回・. txt 出力・コピーができる |
| 7 | 結合確認・失敗系 | タイムアウト・4xx・空コンテンツ・LLM エラーで正しいメッセージが出る |
...
- 生成成功時に POST /api/runs で保存するか、generate 内で保存するか、どちらかに統一する。
