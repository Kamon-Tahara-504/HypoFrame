...
### middleware / proxy に関するメモ

- Next.js 16 では `middleware.ts` ファイルの利用は将来的に `proxy` への移行が推奨されている。
- 本プロジェクトでは、現状の middleware は「Supabase トークンのリフレッシュのみ」を担っており、認可ロジックは Route Handler 側で完結している。
- 将来の移行方針:
  - Supabase の推奨パターン（公式 docs・サンプル）で proxy への移行例が公開されたタイミングで追随する。
  - まず現状 middleware の挙動（トークンリフレッシュと Cookie 反映）を E2E で固定化し、その後 proxy で同等の処理を表現できるか検証する。
  - 認証必須パス（例: `/dashboard`）を追加する際に、proxy への移行と合わせて検討する。
