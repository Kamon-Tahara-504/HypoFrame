...
### フェーズの効果：宛先の具体化と表形式での再利用

| 機能 | 論理の流れ | もたらす効果 |
|------|------------|--------------|
| **代表者名** | 手紙は「誰宛てか」が明示されていないと、送付・フォローが曖昧になる。HP や IR に記載された代表者・役員名を抽出し、下書きに紐づけることで、**宛先が特定可能**になる。 | 手紙下書きが「誰に届けるか」まで一貫した形で扱え、名刺・DM の宛名や社内共有時の説明がしやすくなる。結果として**下書きの実用性（そのまま使える度合い）が上がる**。 |
| **CSV エクスポート** | 要約・仮説・手紙は「1行1企業」の表形式にすると、並べ替え・フィルタ・他システムとの突き合わせができる。テキストだけでは表計算や CRM に載せづらい。 | 生成結果を**表として扱える**ようになり、優先度付け・進捗管理・他ツール（Excel 等）との連携がしやすくなる。複数企業を扱う後続フェーズ以降では、**一覧性と分析可能性**の土台になる。 |

**まとめ**: このフェーズは「1企業あたりのアウトプットの完成度」と「結果を業務でどう使うか」を高める。代表者名で宛先を明確にし、CSV で表として扱えるようにすることで、**生成したものの実用性と再利用性**が上がる。

---

### フェーズの効果：仮説の根拠の厚みと説得力
...
**まとめ**: このフェーズは**入力側の拡張**である。IR PDF を取り込むことで、生成される仮説・提案が「HP のみ」の場合より根拠が厚くなり、**炎上・誤認防止**（断定を避ける方針）と両立したまま、**提案の信頼性**を高められる。

---

### フェーズの効果：対象企業の拡大とリスト運用の効率化
...
**まとめ**: このフェーズは**入力の入口を広げる**機能である。検索ベースのリスト作成と一括出力により、「候補の発見 → 仮説生成 → 結果の整理」がつながり、**スケール（対象企業数の増加）と運用効率**が両立する。

---

### フェーズの効果：既存業務フローへの組み込みと共有のしやすさ
...
**まとめ**: このフェーズは**出力先を既存の業務環境に合わせる**機能である。CSV で「表として扱う」ことはできるが、**日常的に Sheet / Docs を使っている組織では、そこに直接書き込める方が受け入れられやすい**。その結果、**採用率と定着率**の向上が期待できる。
...
**まとめ**: このフェーズは情報源の拡張である。動画 URL と Google 上の企業情報を追加し、仮説の質を底上げする。
...
1. 代表者名を追加してから、CSV に代表者名を含め、CSV エクスポートを用意する。
2. IR 情報 PDF を実装する。「要約・仮説・手紙・代表者名・IR（PDF）」が揃う。
3. リスト作成機能を実装する。代表者名入り CSV が一括出力で使える。
4. Google スプレッドシート / ドキュメント出力を実装する。CSV に加えて Sheet/Docs を出力先として選べるようにする。
5. 動画 URL と Google 企業情報を追加する。
...
**IR 情報 PDF フェーズ（IR 要約を別フィールドで持つ場合）**
...
| 変数名 | フェーズ種別 | 説明 |
|--------|--------------|------|
| `GOOGLE_CSE_API_KEY` または `GOOGLE_API_KEY` | リスト作成・情報拡張 | Google Custom Search API キー |
| `GOOGLE_CSE_CX` | リスト作成・情報拡張 | Custom Search エンジン ID |
| `GOOGLE_CLIENT_ID` | Google 連携 | Google OAuth 2.0 クライアント ID |
| `GOOGLE_CLIENT_SECRET` | Google 連携 | Google OAuth 2.0 クライアントシークレット（サーバー側のみ） |
| `NEXT_PUBLIC_APP_URL` 等 | Google 連携 | OAuth コールバックのベース URL |

### 型・API 契約の変更一覧

| 対象 | フェーズ種別 | 変更内容 |
|------|--------------|----------|
| `types/generate.ts` `GenerateResponse` | 代表者名・CSV | `decisionMakerName?: string \| null` 追加 |
| `types/run.ts` `Run`, `RunInsert` | 代表者名・CSV | `decisionMakerName: string \| null` 追加 |
| `lib/export.ts` | 代表者名・CSV | `buildExportCsv(...)`, `buildExportText` に `decisionMakerName` を渡す。CSV 用ファイル名関数。 |
| `types/generate.ts` `GenerateResponse` | IR 情報 PDF | `irSummary?: string \| null` 追加（別フィールドで返す場合） |
| `types/run.ts` `Run`, `RunInsert` | IR 情報 PDF | `irSummary: string \| null` 追加（同上） |
| 新規 `types/search.ts` 等 | リスト作成 | 検索 API レスポンス型 `{ items: { title, link, snippet }[] }` |
| `types/generate.ts` `GenerateResponse` | 情報拡張 | `videoUrls?: string[]` 追加 |
| `types/run.ts`（runs に保存する場合） | 情報拡張 | `videoUrls: string[] \| null` 追加 |
...
- **MVP の範囲**: 新規スプレッドシート・新規ドキュメントの作成のみ。既存シートへの追記は対象外。トークンは HTTP-only 暗号化 Cookie で保持（DB テーブルは追加しない）。
