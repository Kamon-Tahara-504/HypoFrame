...
- 単純な列挙（「〜しています、〜しています」の繰り返し）は避ける。`,
    ...
    {
      role: "user",
      content: `以下の企業HPから取得したテキスト（構造化済みの場合は ## 会社概要・## 事業内容 等の見出しで区切られています。また「## Google 上の企業情報（補足）」として検索結果・Wikipedia 等の抜粋が含まれる場合があります）から、companyName（企業名）、industry（大まかな業種カテゴリ）、employeeScale（従業員規模）、summaryBusiness（事業展開・3〜5文で具体的に）を抽出し、JSON形式のみで出力してください。
...
- 「課題があります」と述べたうえで、その課題が何か（仮説5段の「潜在課題の仮説」「課題の背景要因」を要約した形で）をはっきり書いてください。現状から考えられる仮説として思い切って述べてよいが、誇張表現は避けてください。そのうえで改善機会・提案仮説につなげる流れにしてください。${focusHint}`,
    },
    {
      role: "user",
